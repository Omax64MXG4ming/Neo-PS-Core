...
class mainLib {
	...
	/**
 * Resuelve una canción siguiendo este orden:
 *
 * 1. Newgrounds
 * 2. Neo PS / DB
 * 3. Custom Music Libraries mediante ids.json
 *
 * Newgrounds solamente permite continuar si responde
 * HTTP 403 o 404.
 */
public function resolveMusicURL($musicID) {

    $musicID = trim((string)$musicID);
     ...
     * ============================================================
     *
     * Comprobamos primero la página de audio.
     *
     * Si responde 2xx -> usamos Newgrounds.
     * Si responde 403/404 -> continuamos.
     * Si responde otro error -> no asumimos que la canción
     * no existe.
     

    $newgroundsURL =
        'https://www.newgrounds.com/audio/listen/' .
        rawurlencode($musicID);

    $ch = curl_init($newgroundsURL);
    ...
    curl_setopt_array($ch, [
        ...
        CURLOPT_TIMEOUT => 10,
        CURLOPT_USERAGENT =>
            'Mozilla/5.0 (Neo PS Music Resolver)',
        CURLOPT_HTTPHEADER => [
     ...
     * Newgrounds existe.
     *
     * Aceptamos cualquier respuesta 2xx.
    ...
    /*
     * Solamente 403 y 404 permiten continuar.
...
*/
    ...
    try {
        ...
        $dbURL = $this->getSongInfo(
            $musicID,
            'download'
        );

        if(is_string($dbURL)) {
            $dbURL = trim(urldecode($dbURL));
        } else {
            $dbURL = '';
        }
    ...
    /*
     * ============================================================
     * 3. CUSTOM MUSIC LIBRARIES
     * ============================================================
     *
     * Aquí NO ponemos URLs de Geometry Dash, Song File Hub,
     * etc.
     *
     * getLibrarySongInfo() consulta:
     *
     * ids.json
     *      ↓
     * server
    ...
    /*
     * No se encontró en ningún origen.
     */
    return false;
}
...
public function updateLibraries($token, $expires, $mainServerTime, $type = 0) {
	...
	if(!isset($customLibrary)) {
		$customLibrary = [
			[1, 'Geometry Dash', 'https://geometrydashfiles.b-cdn.net', 2],
			[3, $gdps, null, 2]
		];
	}
	...
	foreach($customLibrary AS $library) {
		...
		if(
			($types[$type] == 'sfx' && isset($library[3]) && $library[3] === 1) ||
			($types[$type] == 'music' && isset($library[3]) && $library[3] === 0)
		) {
			continue;
		}
	...
	foreach($servers AS $key => &$server) {
		...
		/*
		 * Nombre local del archivo de origen.
		 *
		 * Música oficial:
		 * musiclibrary_version_02.txt
		 * musiclibrary_02.dat
		...
		if($type == 1 && $key == 's1') {
			...
			$versionFile = 'musiclibrary_version_02.txt';
			$dataFile = 'musiclibrary_02.dat';
		...
		/*
		 * Descargar como máximo una vez cada 10 minutos.
		 */
		if(
			isset($oldVersion[1]) &&
			(int)$oldVersion[1] + 600 > time()
		) {
			continue;
		}
		...
		$versionResponse = curl_exec($curl);
		$curlStatus = curl_getinfo($curl, CURLINFO_HTTP_CODE);

		curl_close($curl);

		if($versionResponse === false || $curlStatus < 200 || $curlStatus >= 400) {
			continue;
		}

		$newVersion = (int)trim($versionResponse);

		/*
		 * Solo descargar si existe una versión nueva.
		 */
		if($newVersion > (int)$oldVersion[0]) {
	...
	/*
	 * Versión de la Music Library generada por Neo PS.
	 *
	 * Esto NO sustituye la versión original de Geometry Dash.
	 *
	 * gdps.txt = versión/fecha de nuestra librería generada.
	 */
	$gdpsVersionFile =
		__DIR__ .
		'/../../' .
	 ...
	 * Regenerar si:
	 *
	 * - cambió la versión de Neo PS
	 * - se descargó una librería externa nueva
	 */
	if(
		(int)$oldVersion < (int)$mainServerTime ||
		$updatedLib
	) {
...
public function generateDATFile($mainServerTime, $type = 0) {
	...
	if(
		file_exists(
			__DIR__ .
			'/../../config/skipSFXIDs.json'
		)
	) {

		$skipSFXIDs = json_decode(
			file_get_contents(
				__DIR__ .
				'/../../config/skipSFXIDs.json'
			),
	...
	foreach($servers AS $key => $server) {
		 ...
		 * musiclibrary_02.dat
		 *
		 * SFX y otras librerías conservan
		 * su nombre original.
		 */
		if($type == 1 && $server == 1) {

			$sourceFile = 'musiclibrary_02.dat';
		...
		$res = file_get_contents($sourcePath);

		$res = mb_convert_encoding(
			$res,
			'UTF-8',
			'UTF-8'
		);
	...
	/*
	 * ==========================================================
	 * SFX LOCALES DEL GDPS
	 * ==========================================================
	 */
	if(!$type) {

		$sfxs = $db->prepare("
			SELECT sfxs.*, accounts.userName
			FROM sfxs
			JOIN accounts
				ON accounts.accountID = sfxs.reuploadID
		");
		...
		$encrypted =
			$version .
			";" .
			implode(';', $filesEncrypted) .
			"|" .
			implode(';', $creditsEncrypted) .
			';';
	...
	/*
	 * ==========================================================
	 * MUSIC LOCAL DEL GDPS
	 * ==========================================================
	 */
	} else {

		$songs = $db->prepare("
			SELECT songs.*, accounts.userName
			FROM songs
			JOIN accounts
				ON accounts.accountID = songs.reuploadID
			WHERE isDisabled = 0
		");
		...
		foreach($songs AS &$customSongs) {
			...
			/*
			 * Crear tag del usuario.
			 */
			if(
				empty(
					$accIDs[
						$customSongs['reuploadID']
					]
				)
			) {
			...
			= [
				...
				'externalLink' =>
					urlencode(
						$customSongs['download']
					),

				'new' =>
					(
						$customSongs['reuploadTime'] >
						time() - 604800
						? 1
						: 0
					),
		...
		$encrypted =
			$version .
			"|" .
			implode(';', $authorsEncrypted) .
			";|" .
			implode(';', $songsEncrypted) .
			";|" .
			implode(';', $tagsEncrypted) .
			';';
	...
	$encrypted =
		ExploitPatch::url_base64_encode(
			$encrypted
		);
...
public function getLibrarySongInfo($id, $type = 'music', $extraLibrary = false) {
	...
	foreach($customLibrary AS $customLib) {
		...
		$serverNames[$libraryID] =
			$customLib[1];

		$serverTypes[$libraryID] =
			isset($customLib[3])
			? (int)$customLib[3]
			: 2;
	}
	...
	/*
	 * El ID público puede llegar como string.
	 */
	 ...
	 * ID original     = 12345
	 *
	 * NUNCA debemos usar 8003283 como ID de la canción
	 * de Neo PS.
	 *
	 */

	if(!isset($library['IDs'][$id])) {
		return false;
	}
	...
	if($type === 'music') {
		 ...
		 * IMPORTANTE:
		 *
		 * originalID es el ID real dentro del servidor.
		 *
		 * Si no existe, usamos el ID público solamente como
		 * fallback para compatibilidad con entradas antiguas.
		 */

		$originalID =
			isset($song['originalID']) &&
			(string)$song['originalID'] !== ''
			? (string)$song['originalID']
			: $id;
		 ...
		 * ======================================================
		 *
		 * No usamos server = 3 directamente.
		 *
		 * El servidor local es el que tenga URL NULL.
		 */
		...
		/*
		 * ======================================================
		 * NEO PS / CANCIÓN LOCAL
		 * ======================================================
		 *
		 * Se busca usando ORIGINAL ID.
		 *
		 * Ejemplo:
		 *
		 * ID público:
		 ...
		 *
		 * Archivo:
		 *     dashboard/songs/12345.ogg
		 *
		 * NO:
		 *     dashboard/songs/8003283.ogg
		 */
		...
		if($isLocalServer) {
			...
			/*
			 * Si no encontramos una extensión conocida,
			 * buscamos ID.*
			 */
		 ...
		 * ------------------------------------------------------
		 *
		 * Si la biblioteca proporciona un externalLink válido,
		 * lo usamos primero.
		 *
		 * Esto es importante para servidores como Song File Hub,
		 * donde el formato de descarga puede ser diferente.
		 */
		...
		elseif(
			...
			/*
			 * externalLink de ids.json
			 */

			$externalLink =
				isset($song['externalLink'])
				? trim(
					(string)$song['externalLink']
				)
				: '';


			/*
			 * Si viene URL absoluta, utilizarla.
			 */
			...
			if(
				...
				/*
				 * Fallback para servidores compatibles con
				 * el sistema estándar de GDPS.
				 */
				...
				$link =
					$serverURL .
					'/music/' .
					rawurlencode($originalID) .
					'.ogg?token=' .
					rawurlencode($token) .
					'&expires=' .
					rawurlencode($expires);
			}
		}
		...
		/*
		 * ------------------------------------------------------
		 * 3. SERVIDOR CONOCIDO PERO SIN ARCHIVO
		 * ------------------------------------------------------
		 *
		 * IMPORTANTE:
		 *
		 * NO hacemos fallback a Newgrounds.
		 *
		 * La existencia de la entrada en ids.json significa
		 * que esta canción ya pertenece a una biblioteca.
		 */
		...
		return [
			...
			'serverType' =>
				$serverTypes[$server] ?? 0,

			/*
			 * ID público que conoce el cliente.
			 */
			...
			/*
			 * ID real dentro de la biblioteca.
			 */
			...
			'priorityOrder' =>
				$song['priorityOrder'] ?? 0,

			'isLocalSong' =>
				($localDownload !== ''),

			/*
			 * Indica que ids.json reconoce esta canción.
			 *
			 * El handler puede usar esto para evitar cualquier
			 * fallback accidental a Newgrounds.
			 */
	 ...
	 * ==========================================================
	 *
	 * Esta parte conserva el funcionamiento anterior.
	 */
